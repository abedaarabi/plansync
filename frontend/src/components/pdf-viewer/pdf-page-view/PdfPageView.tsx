...
/**
 * Single PDF page in the sheet viewer: rasterized PDF + SVG overlays + pointer routing for
 * measure, calibrate, takeoff, annotations, and collab. Large on purpose — behavior is intertwined.
 * Extracted pieces live alongside: `coordHelpers` (normalized coords), `snapHelpers` (geometry
 * snap). Dimension line art lives in `PdfPageScreenSvg`.
 */
...
import {
  MEASURE_LINE_POINT_HIT_PX,
  MEASURE_LINE_SEGMENT_HIT_PX,
  PRINT_PDF_SCALE,
} from "./constants";
import type { MeasureLineDrag, PdfPageViewProps, ResizeSession } from "./types";
import {
  clampNorm,
  diamondPointsFromRectCorners,
  distNormPointToSegmentPx,
  distNormPx,
  normFromEvent,
  overlayCssSizePx,
} from "./coordHelpers";
import {
  measureOrthoEnd,
  snapMeasureLineEndpoint,
  snapNorm,
  snapStrokeContext,
} from "./snapHelpers";
import { PdfCollabRemoteCursorsOverlay } from "./PdfCollabRemoteCursorsOverlay";
import { PdfPageScreenSvg } from "./PdfPageScreenSvg";
...
import { signedPerpendicularOffsetPdf } from "@/lib/measureGeometry";
...
import { findNearestSegment } from "@/lib/snap";
...
export function PdfPageView({
  ...
  const [calibrateDialogPointer, setCalibrateDialogPointer] = useState<{
    x: number;
    y: number;
  } | null>(null);
  ...
  const cancelInteraction = useCallback(() => {
    ...
    setCalibrateDialogPointer(null);
    ...
    (e: PointerEvent<HTMLDivElement>) => {
      ...
      if (e.pointerType === "touch" && scrollContainerRef?.current) {
        ...
        if (tool === "pan") {
          ...
          tg.panStartScrollLeft = sc.scrollLeft;
          tg.panStartScrollTop = sc.scrollTop;
          e.preventDefault();
          return;
        }
      }
      ...
      /** Measure, takeoff & calibrate: snap by default. Hold Alt for raw (unsnapped) placement. */
      const sn =
        (tool === "measure" || tool === "takeoff" || tool === "calibrate") && e.altKey
      ...
      if (tool === "calibrate") {
        ...
        if (draft.length === 1) {
          ...
          setCalibrateDialogPointer({ x: e.clientX, y: e.clientY });
    ...
    (e: PointerEvent<HTMLDivElement>) => {
      ...
      if (e.pointerType === "touch" && scrollContainerRef?.current) {
        ...
        if (
          ...
          sc.scrollTop = tg.panStartScrollTop - dy;
          e.preventDefault();
          return;
        }
      }
      ...
      const hoverNormPx = st.snapRadiusPx;
  ...
  // --- Render tree: fixed canvas, interactive overlay (SVG + handlers), sheet UI dialogs ---
  return (
    <>
      <div
        ref={(el) => {
      ...
      >
        ...
        >
          ...
          {!compareReferenceOnly && (
            <Fragment>
              <PdfPageScreenSvg
                ...
                cssW={cssW}
                cssH={cssH}
                layerHighlightSegments={layerHighlightSegments}
                toolbarHoveredLayerId={toolbarHoveredLayerId}
                snapHoverHighlightSegments={snapHoverHighlightSegments}
                snapHoverPathIndex={snapHoverPathIndex}
                scale={scale}
                ...
                takeoffSelectedItemId={takeoffSelectedItemId}
                takeoffHoverZoneId={takeoffHoverZoneId}
                takeoffHoverItemId={takeoffHoverItemId}
                takeoffMoveZoneId={takeoffMoveZoneId}
                tool={tool}
                takeoffVertexEditZoneId={takeoffVertexEditZoneId}
                takeoffDrawKind={takeoffDrawKind}
                takeoffLineStart={takeoffLineStart}
                takeoffLinePreview={takeoffLinePreview}
                takeoffDraftColor={takeoffDraftColor}
                takeoffSnapHint={takeoffSnapHint}
                takeoffAreaMode={takeoffAreaMode}
                takeoffRectAnchor={takeoffRectAnchor}
                takeoffRectPreview={takeoffRectPreview}
                takeoffAreaPts={takeoffAreaPts}
                takeoffAreaPreview={takeoffAreaPreview}
                takeoffCountDraftPoints={takeoffCountDraftPoints}
                countDraftLabelOffset={countDraftLabelOffset}
                selectMarquee={selectMarquee}
                zoomMarquee={zoomMarquee}
                remoteCollabSelectionRects={remoteCollabSelectionRects}
                selectionBounds={selectionBounds}
                markupHoverHighlight={markupHoverHighlight}
                selectionResizeHandles={selectionResizeHandles}
                brushHoverNorm={brushHoverNorm}
                ...
                rectDrag={rectDrag}
                polygonMarkup={polygonMarkup}
                polygonPreview={polygonPreview}
                measureKind={measureKind}
                measureStart={measureStart}
                measureEnd={measureEnd}
                measurePreview={measurePreview}
                measureOffsetPdf={measureOffsetPdf}
                calibrationByPage={calibrationByPage}
                measureLabelColor={measureLabelColor}
                measureLabelFontSize={measureLabelFontSize}
                measureUnit={measureUnit}
                pageW={pageSize.w}
                pageH={pageSize.h}
                measureMultiPoints={measureMultiPoints}
                calibrateDraft={calibrateDraft}
                calibrateOpen={calibrateOpen}
                calibratePreview={calibratePreview}
                calibrateTargetMm={calibrateTargetMm}
              />
          ...
          <PdfCollabRemoteCursorsOverlay
            pageIdx0={pageIdx0}
            viewerCollab={viewerCollab}
            collabPeerByUserId={collabPeerByUserId}
          />
      ...
      <CalibrateDialog
        ...
        anchorNorm={
          calibrateOpen && calibrateDraft.length >= 2
            ? { x: calibrateDraft[1].x, y: calibrateDraft[1].y }
            : null
        }
        pointerClient={calibrateOpen ? calibrateDialogPointer : null}
        onCancel={() => {
          setCalibrateOpen(false);
          setCalibrateDialogPointer(null);
          setCalibrateDraft([]);
