...
export function CommittedAnnotationsSvg({
  ...
  const arrowAnns = annotations.filter(
    (a) => a.type === "line" && a.arrowHead && a.points.length === 2,
  );

  return (
    <>
      <defs>
        {/* Per-annotation markers: SVG markers do not inherit stroke/currentColor reliably (esp. Safari). */}
        {arrowAnns.map((a) => (
          <marker
            key={a.id}
            id={`${arrowMarkerId}-${a.id}`}
            markerWidth="10"
            markerHeight="7"
            refX="9"
            refY="3.5"
              ...
              >
                <>
                  ...
                  <line
                    ...
                    strokeWidth={lw}
                    strokeLinecap="round"
                    markerEnd={a.arrowHead ? `url(#${arrowMarkerId}-${a.id})` : undefined}
                    vectorEffect={a.fromSheetAi ? "non-scaling-stroke" : undefined}
                  />
                </>
              </RotatedMarkupG>
