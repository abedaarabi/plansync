...
export function FileCommentsPanel({ projectId, fileId, fileVersionId, currentUserId }: Props) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3">
      <div className="rounded-xl border border-[var(--enterprise-border)] bg-white p-3">
        ...
        <div className="mt-2 flex flex-col gap-2 sm:flex-row">
          <textarea
            ...
            className="min-h-24 flex-1 rounded-xl border border-[var(--enterprise-border)] bg-white px-3 py-2 text-sm text-[var(--enterprise-text)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--enterprise-primary)]"
          />
          <button
            type="button"
      ...
      <div className="enterprise-scrollbar min-h-0 max-h-[48dvh] flex-1 overflow-y-auto rounded-xl border border-[var(--enterprise-border)] bg-[var(--enterprise-surface)] p-2">
        {isPending ? (
          ...
          <ul className="space-y-2">
            {comments.map((comment) => (
              <li
                key={comment.id}
                className="rounded-xl border border-[var(--enterprise-border)] bg-white p-3 shadow-[var(--enterprise-shadow-xs)]"
              >
