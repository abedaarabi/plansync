...
export function SocialAuthButtons({ callbackURL, onError }: Props) {
  const [pending, setPending] = useState<SocialProviderId | null>(null);
  const ids = getEnabledSocialProviders();
      ...
      <div className="flex flex-col gap-2.5">
        ...
        {ids.includes("github") && (
          <button
            ...
            className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-800 bg-slate-900 py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400/40 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {pending === "github" ? (
              <Loader2 className="h-5 w-5 shrink-0 animate-spin text-slate-300" aria-hidden />
            ) : (
        ...
        {ids.includes("slack") && (
          <button
            type="button"
            disabled={pending !== null}
            onClick={() => void handle("slack")}
            className="flex w-full items-center justify-center gap-3 rounded-xl border border-[#611f69] bg-[#4A154B] py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-[#611f69] focus:outline-none focus:ring-2 focus:ring-purple-400/40 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {pending === "slack" ? (
              <Loader2 className="h-5 w-5 shrink-0 animate-spin text-white/80" aria-hidden />
            ) : (
