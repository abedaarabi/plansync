  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl px-6">
          ...
          <div className="grid gap-8 lg:grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)] lg:items-start">
            <div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <Link
                  href="/use-cases"
                  className="inline-flex items-center gap-2 rounded-xl border border-slate-200/70 bg-white/70 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 hover:text-slate-900"
                >
                  {t("useCasesCta")}
                </Link>
              </div>
        ...
        <AnimateIn delay={80}>
          <div className="mt-8 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-[0_10px_30px_-22px_rgba(15,23,42,0.28)] sm:p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {["NORTHRIDGE BUILD", "MECHANICA PRO", "HARBOR FM"].map((name) => (
                  <span
                    key={name}
                    className="inline-flex rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-semibold tracking-wide text-slate-600"
                  >
                    {name}
                  </span>
                ))}
