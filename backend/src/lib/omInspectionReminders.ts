...
function inspectionsAppHref(projectId: string): string {
  return `/projects/${projectId}/om/inspections`;
}

function notificationBody(items: OmInspectionReminderRow[]): string {
...
function buildDigestTableHtml(items: OmInspectionReminderRow[], appOrigin: string): string {
  const sorted = [...items].sort((a, b) => a.nextDueAt.getTime() - b.nextDueAt.getTime());
...
function buildReminderEmail(
  ...
  const text = [
    ...
    ...sorted.map(
      (it) =>
        `- ${it.projectName}: ${it.name} — due ${it.nextDueAt.toISOString().slice(0, 10)} (${it.overdue ? "overdue" : "due soon"})`,
    ),
    "",
    `Open: ${actionUrl}`,
  ].join("\n");
...
export async function runOmInspectionReminders(env: Env): Promise<OmDigestRunStats> {
  ...
  const byWorkspace = new Map<string, OmInspectionReminderRow[]>();
  for (const t of templates) {
    ...
    if (!settings.modules.omInspections) continue;

    const nd = t.nextDueAt!;
    const overdue = nd < startToday;

    // fallow-ignore-next-line code-duplication
    const row: OmInspectionReminderRow = {
      templateId: t.id,
  ...
  for (const [workspaceId, items] of byWorkspace) {
    ...
    for (const member of members) {
      if (!isWorkspaceManagerRole(member.role)) continue;
