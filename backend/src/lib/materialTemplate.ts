...
export function parseMaterialTemplateJson(raw: unknown): MaterialTemplate {
  ...
  const fields: MaterialTemplateField[] = [];
  for (const item of fieldsIn) {
    ...
    const tr = f.type;
    const type: MaterialCustomFieldType = tr === "number" || tr === "currency" ? tr : "text";
    const required = Boolean(f.required);
    const order =
      typeof f.order === "number" && Number.isFinite(f.order) ? Math.floor(f.order) : fields.length;
...
export type MaterialTemplateParseResult =
  | { ok: true; template: MaterialTemplate }
  | { ok: false; error: string };

export function parseMaterialTemplatePatchBody(body: unknown): MaterialTemplateParseResult {
  ...
  for (const f of parsed.data.fields) {
    ...
    seenKeys.add(key);
    fields.push({
      id: (f.id?.trim() || randomUUID()) as string,
      key,
      label: f.label.trim(),
      type: f.type,
  ...
  fields.sort(
    (a, b) => a.order - b.order || a.key.localeCompare(b.key, undefined, { sensitivity: "base" }),
  );
  return {
    ok: true,
    template: {
...
export function templateToDbJson(t: MaterialTemplate): Prisma.InputJsonValue {
  return {
    version: t.version,
...
export type NormalizeAttrsResult =
  | { ok: true; attributes: Prisma.InputJsonValue }
  | { ok: false; error: string };
...
export function normalizeCustomAttributes(
  ...
  return { ok: true, attributes: out as Prisma.InputJsonValue };
}
...
export function jsonObjectForResponse(
