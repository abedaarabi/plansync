...
import type { BimClash, BimClashStatus, BimClashType, Prisma } from "@prisma/client";
...
export function canonicalizeElementPair(
  a: { elementId: string; guid: string; fileVersionId: string },
  b: { elementId: string; guid: string; fileVersionId: string },
): CanonicalPair {
...
export function shouldReopenDismissed(
  status: BimClashStatus,
  statusDistanceMm: number | null | undefined,
  newDistanceMm: number,
  materialityMm = CLASH_MATERIALITY_MM,
): boolean {
  if (status !== "RESOLVED" && status !== "IGNORED") return false;
  if (statusDistanceMm == null || !Number.isFinite(statusDistanceMm)) return false;
  return Math.abs(newDistanceMm - statusDistanceMm) > materialityMm;
}

export function pairKey(elementAId: string, elementBId: string): string {
  return `${elementAId}|${elementBId}`;
}

type PointLike = BimClashPoint;

function distSq(a: PointLike, b: PointLike): number {
  const dx = a.x - b.x;
  const dy = a.y - b.y;
  const dz = a.z - b.z;
...
export function assignClashGroups(
  items: { id: string; point: PointLike; groupId: string | null }[],
  radiusM = CLASH_GROUP_RADIUS_M,
): Map<string, string> {
  const r2 = radiusM * radiusM;
  const result = new Map<string, string>();
  const clusters: { groupId: string; members: typeof items }[] = [];

  for (const item of items) {
    let matched: (typeof clusters)[number] | null = null;
    ...
    if (matched) {
      ...
      if (item.groupId) {
        ...
        if (existingPreferred) matched.groupId = existingPreferred;
      }
    } else {
      clusters.push({
  ...
  for (const cluster of clusters) {
    const preferred =
      cluster.members.map((m) => m.groupId).find((g): g is string => Boolean(g)) ?? cluster.groupId;
...
export function isNoLongerClashing(
  lastSeenAt: Date,
  testLastRunAt: Date | null | undefined,
): boolean {
  if (!testLastRunAt) return false;
  return lastSeenAt.getTime() < testLastRunAt.getTime();
}

function parsePoint(json: unknown): PointLike {
  if (
    json &&
    typeof json === "object" &&
    "x" in json &&
    "y" in json &&
    "z" in json &&
    typeof (json as PointLike).x === "number" &&
    typeof (json as PointLike).y === "number" &&
    typeof (json as PointLike).z === "number"
  ) {
    return json as PointLike;
  }
  return { x: 0, y: 0, z: 0 };
}

function tradeMatchesDiscipline(trade: string | null | undefined, discipline: string): boolean {
  if (!trade?.trim()) return false;
  const t = trade.toLowerCase();
  const d = discipline.toLowerCase();
  if (t.includes(d) || d.includes(t)) return true;
  if (d === "mechanical" && /mech|hvac|plumb/.test(t)) return true;
  if (d === "electrical" && /elec/.test(t)) return true;
  if (d === "structure" && /struct|steel|concrete/.test(t)) return true;
  if (d === "architecture" && /arch/.test(t)) return true;
  if (d === "mep" && /mep|plumb|fire/.test(t)) return true;
  return false;
}

async function resolveTradeAssignee(projectId: string, discipline: string): Promise<string | null> {
...
// fallow-ignore-next-line complexity
async function resolveHitsToElements(
  projectId: string,
  hits: BimClashHit[],
): Promise<ResolvedHit[]> {
  ...
  const guidsByFile = new Map<string, Set<string>>();
  const metaByFileGuid = new Map<string, { name: string | null; ifcType: string | null }>();
  for (const h of hits) {
    const fileA = fvToFile.get(h.fileVersionIdA);
    const fileB = fvToFile.get(h.fileVersionIdB);
    if (!fileA || !fileB) continue;
    if (!guidsByFile.has(fileA)) guidsByFile.set(fileA, new Set());
    if (!guidsByFile.has(fileB)) guidsByFile.set(fileB, new Set());
  ...
  const elementByFileGuid = new Map<
    string,
    { id: string; ifcType: string | null; name: string | null }
  >();
  ...
  for (const h of hits) {
    ...
    if (!elA || !elB) continue;
    const pair = canonicalizeElementPair(
      { elementId: elA.id, guid: h.guidA, fileVersionId: h.fileVersionIdA },
      { elementId: elB.id, guid: h.guidB, fileVersionId: h.fileVersionIdB },
    );
...
export async function reconcileClashRun(args: {
  ...
  const bestByPair = new Map<string, ResolvedHit>();
  ...
  for (const hit of bestByPair.values()) {
    ...
    if (!prev) {
      ...
      await prisma.bimClash.create({
        data: {
          ...
          pointJson: hit.point as Prisma.InputJsonValue,
    ...
    const data: Prisma.BimClashUpdateInput = {
      clashType: hit.clashType,
      distanceMm: hit.distanceMm,
      pointJson: hit.point as Prisma.InputJsonValue,
      contactCount: hit.contactCount,
  ...
  const groupMap = assignClashGroups(
    all.map((c) => ({
      id: c.id,
  ...
  await prisma.bimClashTest.update({
    where: { id: testId },
    data: {
      setAJson: payload.setA as unknown as Prisma.InputJsonValue,
      setBJson: payload.setB as unknown as Prisma.InputJsonValue,
...
export function parseSetDef(json: unknown): BimClashSetDef {
  if (
    json &&
    typeof json === "object" &&
    "label" in json &&
    typeof (json as BimClashSetDef).label === "string" &&
    Array.isArray((json as BimClashSetDef).rules)
  ) {
    return json as BimClashSetDef;
  }
  return { label: "Set", rules: [] };
}

export function parseRunStats(json: unknown): BimClashRunStats | null {
  if (!json || typeof json !== "object") return null;
  const s = json as Partial<BimClashRunStats>;
  if (
    typeof s.newCount !== "number" ||
