import type { BimElementChangeType } from "@prisma/client";
...
function searchableAttrs(entry: BimQuantityEntry): { key: string; value: string }[] {
  ...
  if (entry.quantities.area != null) push("area", String(entry.quantities.area));
  if (entry.quantities.volume != null) push("volume", String(entry.quantities.volume));
  if (entry.quantities.length != null) push("length", String(entry.quantities.length));
  return out;
}

async function ensureMetadataOnS3(
  env: Env,
  workspaceId: string,
  hash: string,
  ...
  if (existing) return existing.metadataS3Key;
...
async function loadPriorGuidHashes(
  fileId: string,
  priorFileVersionId: string,
): Promise<Map<string, { hash: string; s3Key: string; elementId: string }>> {
  ...
  const map = new Map<string, { hash: string; s3Key: string; elementId: string }>();
...
export async function persistElementVersionDiff(opts: {
...
}): Promise<ElementDiffStats> {
  const stats: ElementDiffStats = { added: 0, unchanged: 0, modified: 0, deleted: 0 };
  const priorMap = opts.priorFileVersionId
    ? await loadPriorGuidHashes(opts.fileId, opts.priorFileVersionId)
    : new Map<string, { hash: string; s3Key: string; elementId: string }>();
  ...
  for (const entry of opts.elements) {
    ...
    let changeType: BimElementChangeType;
    let metadataS3Key: string;
...
export async function cloneElementVersionsFromPrior(
  priorFileVersionId: string,
