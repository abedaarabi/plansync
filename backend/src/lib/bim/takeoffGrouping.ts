...
export type BimCostGroupSource = "typeName" | "ifcType";
...
export function costGroupForEntry(entry: BimQuantityEntry): {
  key: string;
  label: string;
  source: BimCostGroupSource;
} {
...
export function groupEntriesForCostTakeoff(entries: BimQuantityEntry[]): BimCostGroup[] {
  ...
  for (const entry of entries) {
    ...
    if (!group.ifcType && entry.ifcType) group.ifcType = entry.ifcType;
  }
  return [...map.values()].sort(
    (a, b) => b.guids.length - a.guids.length || a.label.localeCompare(b.label),
  );
}
