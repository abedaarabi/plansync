...
export type BimModelReadyEmailInput = {
  to: string;
  fileName: string;
  projectName: string;
...
export function buildBimModelReadyEmailHtml(env: Env, input: BimModelReadyEmailInput): string {
  if (input.failed) {
  ...
  if (input.loqLine) lines.push(input.loqLine);
...
export function buildBimModelReadyEmailText(input: BimModelReadyEmailInput): string {
  if (input.failed) {
...
export function buildBimViewerUrl(
  env: Env,
  params: {
    projectId: string;
    fileId: string;
...
export function buildProjectFilesUrl(env: Env, projectId: string): string {
  const base = env.PUBLIC_APP_URL.replace(/\/$/, "");
  return `${base}/projects/${encodeURIComponent(projectId)}/files`;
}

export function buildPublishWizardPath(projectId: string, fileVersionId: string): string {
