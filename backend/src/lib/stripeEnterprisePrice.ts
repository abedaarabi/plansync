...
function assertLooksLikeStripePriceId(id: string): void {
  ...
  if (lower === "sk_" || lower === "rk_" || id.startsWith("pk_")) {
    throw new Error(
      "STRIPE_PRICE_ENTERPRISE_MONTHLY must be a Price id (price_… from Stripe Dashboard), not an API key. Put sk_test_… / sk_live_… only in STRIPE_SECRET_KEY. Or remove STRIPE_PRICE_ENTERPRISE_MONTHLY to auto-create the $99/mo price.",
    );
  }
  if (!id.startsWith("price_")) {
    throw new Error(
      "STRIPE_PRICE_ENTERPRISE_MONTHLY should look like price_xxxxxxxx (Product catalog → your price). Remove it to use the built-in $99/mo price instead.",
    );
  }
}
 ...
 */
export async function resolveEnterpriseMonthlyPriceId(
  stripe: Stripe,
