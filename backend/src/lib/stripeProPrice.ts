...
function assertLooksLikeStripePriceId(id: string): void {
  ...
  if (lower === "sk_" || lower === "rk_" || id.startsWith("pk_")) {
    throw new Error(
      "STRIPE_PRICE_PRO_MONTHLY must be a Price id (price_… from Stripe Dashboard), not an API key. Put sk_test_… / sk_live_… only in STRIPE_SECRET_KEY. Or remove STRIPE_PRICE_PRO_MONTHLY to auto-create the $49/mo price.",
    );
  }
  if (!id.startsWith("price_")) {
    throw new Error(
      "STRIPE_PRICE_PRO_MONTHLY should look like price_xxxxxxxx (Product catalog → your price). Remove it to use the built-in $49/mo price instead.",
    );
  }
}
