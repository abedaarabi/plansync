...
export type InspectionRunForReportPdf = {
  id: string;
  status: string;
  resultJson: unknown;
  createdAt: Date;
...
function outcomeColors(o: string): { fg: string; bg: string } {
  if (o === "pass") return { fg: C.pass, bg: C.passBg };
  if (o === "fail") return { fg: C.fail, bg: C.failBg };
  if (o === "na") return { fg: C.na, bg: C.naBg };
  return { fg: C.faint, bg: C.surface };
}

function personLabel(p: { name: string | null; email?: string | null } | null | undefined): string {
  if (!p) return "—";
  const n = p.name?.trim();
  if (n) return n;
  return p.email?.trim() || "—";
...
export async function buildInspectionReportPdfBuffer(
  run: InspectionRunForReportPdf,
): Promise<Buffer> {
  const margin = 44;
  const checklistRaw = Array.isArray(run.template.checklistJson)
    ? (run.template.checklistJson as ChecklistItem[])
    : [];
  ...
  for (const item of checklist) {
    ...
    if (oc === "pass") passCount += 1;
    else if (oc === "fail") failCount += 1;
    else if (oc === "na") naCount += 1;
    else unanswered += 1;
    if (typeof res?.photoDataUrl === "string" && res.photoDataUrl.startsWith("data:image")) {
      photoCount += 1;
    }
    if (typeof res?.note === "string" && res.note.trim()) noteCount += 1;
  }
  const total = checklist.length;
  const overall =
    failCount > 0 ? "Deficient" : unanswered > 0 ? "Incomplete" : total > 0 ? "Conforming" : "—";
  ...
  const ensureSpace = (need: number) => {
    if (doc.y + need > pageBottom() - 12) {
      ...
      doc.text(
        `PlanSync · ${run.project.name} · ${run.template.name} (continued)`,
        margin + 8,
        margin - 2,
        { width: contentW - 16 },
      );
  ...
  const byLevel = new Map<string, ChecklistItem[]>();
    ...
    for (const item of items) {
      ...
      const photoFile =
        typeof res?.photoFileName === "string" && res.photoFileName.trim()
          ...
          : "";
      const followUp =
        typeof res?.followUpIssueId === "string" && res.followUpIssueId.trim()
      ...
      const estimated =
        52 + (note ? 22 : 0) + (followUp ? 14 : 0) + (photoBuf ? 170 : 0) + (photoFile ? 12 : 0);
  ...
  doc.text(
    "Generated by PlanSync O&M. Outcomes: Pass = conforming, Fail = deficient (may create work orders), N/A = not applicable.",
    margin + 10,
    noteY,
    { width: contentW - 20 },
  );
