 ...
 */
export async function deleteAllWorkspaceS3Objects(env: Env, workspaceId: string): Promise<void> {
  ...
  if (ws?.logoS3Key) keys.add(ws.logoS3Key);
  ...
  for (const row of proposalPdfs) {
    if (row.pdfS3Key) keys.add(row.pdfS3Key);
  }
