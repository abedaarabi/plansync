...
async function loadPdfAssetForLevel(levelId: string, fileAssetId: string, projectId: string) {
...
export async function assignDrawingToLevel(params: {
  ...
  const { level, file, pdfFv } = await loadPdfAssetForLevel(
    params.levelId,
    params.fileAssetId,
    params.projectId,
  );
  ...
  if (alreadyOnLevel) throw new Error("Drawing already assigned to this level");
  ...
  if (level.buildingId) {
    ...
    if (elsewhere) throw new Error("Drawing is already assigned to another level in this building");
  } else {
    const elsewhere = await prisma.drawingLevelMap.findFirst({
      where: { pdfFileId: file.id, projectId: params.projectId },
      select: { id: true },
    });
    if (elsewhere) throw new Error("Drawing is already assigned to a level");
  }
