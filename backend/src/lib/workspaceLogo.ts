...
function isRasterImageBufferForPdf(buf: Buffer): boolean {
...
function looksLikeSvgBuffer(buf: Buffer): boolean {
 ...
 */
export async function prepareWorkspaceLogoBufferForPdf(buf: Buffer | null): Promise<Buffer | null> {
  if (!buf?.length) return null;
  if (isRasterImageBufferForPdf(buf)) return buf;
  if (!looksLikeSvgBuffer(buf)) return null;
  try {
...
export async function fetchWorkspaceLogoImageBuffer(
  env: Env,
  ws: { logoS3Key: string | null; logoUrl: string | null },
): Promise<Buffer | null> {
  ...
  if (ws.logoS3Key && client && bucket) {
    try {
      ...
      if (bytes.length > MAX_LOGO_FETCH_BYTES) return null;
  ...
  try {
    ...
    if (ab.byteLength > MAX_LOGO_FETCH_BYTES) return null;
