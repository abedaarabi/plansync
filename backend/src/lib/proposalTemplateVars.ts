...
export type ProposalLineForTable = {
  itemName: string;
  quantity: string;
  unit: string;
  rate: string;
...
export function buildTakeoffTableHtml(input: {
  ...
  const th =
    'style="text-align:left;padding:10px 12px;border-bottom:1px solid #e2e8f0;font:600 12px Inter,system-ui,sans-serif;color:#64748b;text-transform:uppercase;letter-spacing:0.04em"';
  const td =
    'style="padding:10px 12px;border-bottom:1px solid #f1f5f9;font:14px Inter,system-ui,sans-serif;color:#0f172a"';
  const tdNum =
    'style="padding:10px 12px;border-bottom:1px solid #f1f5f9;font:14px Inter,system-ui,sans-serif;color:#0f172a;text-align:right;white-space:nowrap"';
...
const VARS: Record<string, (ctx: TemplateReplaceContext) => string> = {
...
export type TemplateReplaceContext = {
  clientName: string;
  ...
  proposalTotalFormatted: string;
  proposalExpiryFormatted: string;
  takeoffTableHtml: string;
};

/** Replace {{var.name}} placeholders in template body. */
export function applyProposalTemplate(body: string, ctx: TemplateReplaceContext): string {
  let out = body;
