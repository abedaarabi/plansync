...
import { z } from "zod";
import {
  OrchestrationApprovalStatus,
  OrchestrationRunStatus,
  type Prisma,
  type WorkspaceRole,
} from "@prisma/client";
import { prisma } from "../../lib/prisma.js";
import { loadProjectWithAuth, type ProjectAuthContext } from "../../lib/permissions.js";

function canManage(ctx: ProjectAuthContext): boolean {
  if (ctx.workspaceMember.isExternal) return false;
  const role = ctx.workspaceMember.role as WorkspaceRole;
  return role === "SUPER_ADMIN" || role === "ADMIN";
}

function canRead(ctx: ProjectAuthContext): boolean {
  return !ctx.workspaceMember.isExternal;
}

export function registerOrchestrationRoutes(r: Hono, needUser: MiddlewareHandler) {
  r.get("/projects/:projectId/job-runs", needUser, async (c) => {
    ...
    if (!canRead(access.ctx)) return c.json({ error: "Forbidden" }, 403);
  ...
  r.post("/projects/:projectId/job-runs", needUser, async (c) => {
    ...
    const access = await loadProjectWithAuth(projectId, c.get("user").id);
    if ("error" in access) return c.json({ error: access.error }, access.status);
    if (!canManage(access.ctx)) return c.json({ error: "Admin or Super Admin only" }, 403);
    const body = z
      .object({
    ...
    const created = await prisma.jobRun.create({
      data: {
        ...
        payloadJson: (body.data.payloadJson ?? null) as Prisma.InputJsonValue,
    ...
    const updated = await prisma.jobRun.update({
      ...
      data: {
        ...
        ...(body.data.errorJson !== undefined
          ? { errorJson: body.data.errorJson as Prisma.InputJsonValue }
          : {}),
      },
    });
    return c.json({
  ...
  r.post("/projects/:projectId/orchestration/workflows", needUser, async (c) => {
    ...
    if (body.data.environmentId) {
      ...
      if (!envRow) return c.json({ error: "Environment not found" }, 400);
    }
