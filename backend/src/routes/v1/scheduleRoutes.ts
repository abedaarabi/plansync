...
function validateForest(tasks: TaskIn[]): { ok: true } | { error: string } {
  ...
  for (const start of tasks) {
    ...
    for (let i = 0; i <= tasks.length; i++) {
      if (seen.has(cur!)) return { error: "Schedule parent links form a cycle" };
      seen.add(cur!);
      const node = byId.get(cur!);
      if (!node?.parentId || !ids.has(node.parentId)) break;
      cur = node.parentId;
    }
  }
  return { ok: true };
  ...
  r.put("/projects/:projectId/schedule", needUser, async (c) => {
    ...
    if (allTakeoffIds.length > 0) {
      ...
      if (found.length !== allTakeoffIds.length) {
        return c.json(
          { error: "One or more takeoff lines are missing or belong to another project" },
          400,
        );
      }
    }
