...
import {
  buildPunchAssignedEmailHtml,
  buildPunchAssignedEmailText,
  buildPunchListPath,
  buildPunchListUrl,
} from "../../lib/punchAssignEmail.js";
import { createUserNotifications } from "../../lib/userNotifications.js";
import {
  buildPunchReferencePhotoKey,
  newUploadId,
  s3KeyMatchesPunchReferencePhoto,
} from "../../lib/fileUpload.js";
import { deleteObject, presignGet, presignPut } from "../../lib/s3.js";
import {
  MAX_PUNCH_PHOTO_BYTES,
  MAX_PUNCH_REFERENCE_PHOTOS,
  parsePunchReferencePhotos,
  punchPhotosStorageBytes,
  punchReferencePhotosToJsonValue,
  type PunchReferencePhotoParsed,
} from "../../lib/punchReferencePhotos.js";
...
async function sendPunchAssignedEmail(
  env: Env,
  input: {
...
export function registerPunchRoutes(r: Hono, needUser: MiddlewareHandler, env: Env) {
  ...
  r.post("/projects/:projectId/punch/templates/:templateId/apply", needUser, async (c) => {
    ...
    if (items.length === 0) return c.json({ error: "Template has no items" }, 400);
  ...
  r.post("/projects/:projectId/punch/:punchId/photos/presign", needUser, async (c) => {
    ...
    if (!punch) return c.json({ error: "Not found" }, 404);
    ...
    if (!ALLOWED_PUNCH_PHOTO_CONTENT_TYPES.has(ct)) {
      return c.json(
        { error: "Only JPEG, PNG, WebP, GIF, or HEIC/HEIF images are allowed for punch photos" },
        400,
      );
    }
    ...
    const key = buildPunchReferencePhotoKey(
      res.project.workspaceId,
      projectId,
      uploadId,
      body.data.fileName,
  ...
  r.post("/projects/:projectId/punch/:punchId/photos/complete", needUser, async (c) => {
    ...
    const entry: PunchReferencePhotoParsed = {
      id: photoId,
      s3Key: body.data.key,
  ...
  r.get("/projects/:projectId/punch/:punchId/photos/:photoId/presign-read", needUser, async (c) => {
    ...
    if (!hit) return c.json({ error: "Photo not found" }, 404);
    let url: string | null;
    try {
  ...
  r.patch("/projects/:projectId/punch/:punchId", needUser, async (c) => {
    ...
    if (refRaw !== undefined) {
      ...
      for (const row of incoming) {
        if (seenIds.has(row.id)) return c.json({ error: "Duplicate photo id" }, 400);
        seenIds.add(row.id);
        if (seenKeys.has(row.s3Key)) return c.json({ error: "Duplicate photo storage key" }, 400);
    ...
    const dueDate =
      body.data.dueDateYmd === undefined
        ? undefined
  ...
  r.post("/projects/:projectId/punch/bulk", needUser, async (c) => {
    ...
    const update: { assigneeId?: string | null; status?: PunchStatus; completedAt?: Date | null } =
      {};
