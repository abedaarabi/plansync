...
import {
  ...
  downloadMicrosoftDriveFile,
  getDropboxOpenUrl,
  getGoogleDriveWebViewUrl,
  getMicrosoftDriveWebUrl,
  exchangeDropboxCode,
  exchangeGoogleCode,
  exchangeMicrosoftCode,
  fetchDropboxAccountLabel,
  fetchGoogleAccountLabel,
  fetchMicrosoftAccountLabel,
  listDropboxFolder,
  listGoogleDriveChildren,
  listMicrosoftDriveChildren,
  oauthRedirectBase,
...
type AuthApi = { api: { getSession: (o: { headers: Headers }) => Promise<unknown> } };
...
function redirectWithError(c: Context, env: Env, message: string) {
  const u = new URL(env.PUBLIC_APP_URL.replace(/\/$/, ""));
  u.searchParams.set("cloud_import_error", message.slice(0, 500));
  return c.redirect(u.toString(), 302);
}

function redirectSuccess(c: Context, env: Env, returnTo: string | null) {
...
export function registerCloudRoutes(r: Hono, needUser: MiddlewareHandler, env: Env, auth: AuthApi) {
  r.get("/cloud/connections", needUser, async (c) => {
    ...
    return c.json({
      connections: rows.map((row) => ({
        provider:
          row.provider === CloudStorageProvider.GOOGLE_DRIVE
            ? "google"
  ...
  r.get("/cloud/google/authorize", needUser, async (c) => {
    ...
    if (!url)
      return c.json(
        { error: "Google Drive import is not configured (GOOGLE_CLIENT_ID/SECRET)" },
        503,
      );
  ...
  r.get("/cloud/microsoft/authorize", needUser, async (c) => {
    ...
    if (!url) {
      return c.json(
        { error: "OneDrive import is not configured (MICROSOFT_CLIENT_ID/SECRET)" },
        503,
      );
    }
    return c.redirect(url, 302);
  ...
  async function handleOAuthCallback(c: Context, provider: "google" | "microsoft" | "dropbox") {
    ...
    if (provider === "google") {
      ...
      if ("error" in ex) return redirectWithError(c, env, ex.error);
    ...
    if (provider === "google") accountLabel = await fetchGoogleAccountLabel(accessToken);
    else if (provider === "microsoft") accountLabel = await fetchMicrosoftAccountLabel(accessToken);
    else accountLabel = await fetchDropboxAccountLabel(accessToken);
  ...
  r.post("/cloud/google/browse", needUser, async (c) => {
    ...
    if ("error" in tok) return c.json({ error: tok.error }, tok.status as ContentfulStatusCode);
    const items = await listGoogleDriveChildren(tok.accessToken, body.data.parentId);
    if ("error" in items) return c.json({ error: items.error }, 502);
  ...
  r.post("/cloud/google/open-link", needUser, async (c) => {
    ...
    if ("error" in link) return c.json({ error: link.error }, 400);
  ...
  r.post("/cloud/google/import", needUser, async (c) => {
    ...
    if ("error" in buf) return c.json({ error: buf.error }, 400);
