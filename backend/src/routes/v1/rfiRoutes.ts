...
import {
  buildRfiAttachmentKey,
  newUploadId,
  s3KeyMatchesRfiAttachment,
} from "../../lib/fileUpload.js";
import {
  buildRfiClosedEmailLines,
  buildRfiMessageEmailLines,
  buildRfiReopenedEmailLines,
  buildRfiOverdueEmailLines,
  buildRfiResponseEmailLines,
  buildRfiSentEmailLines,
  rfiDetailUrl,
  sendRfiNotificationEmail,
} from "../../lib/rfiEmail.js";
import {
  createUserNotifications,
  rfiAssigneeUserIds,
  rfiParticipantUserIds,
} from "../../lib/userNotifications.js";
import {
  isRfiRichTextEffectivelyEmpty,
  rfiRichTextPlainExcerpt,
  sanitizeRfiMessageHtml,
} from "../../lib/sanitizeRfiRichText.js";
...
type RfiLoaded = Prisma.RfiGetPayload<{ include: typeof rfiInclude }>;

function rfiRowJson(row: RfiLoaded) {
...
function isRfiCreator(rfi: { creatorId: string | null }, userId: string): boolean {
  if (!rfi.creatorId) return true;
...
async function resolveDrawingFields(
  ...
  pinNormX: number | null | undefined,
  pinNormY: number | null | undefined,
): Promise<
  | {
      fileId: string | null;
...
async function syncRfiIssueLinks(
...
async function syncRfiAssigneeLinks(
  tx: Prisma.TransactionClient,
  rfiId: string,
  userIds: string[],
): Promise<void> {
...
export function registerRfiRoutes(r: Hono, needUser: MiddlewareHandler, env: Env) {
  ...
  r.post("/projects/:projectId/rfis/:rfiId/messages", needUser, async (c) => {
    ...
    const pushEmail = (e: string | null | undefined) => {
      ...
      if (t) notifySet.add(t);
    };
    pushEmail(existing.creator?.email);
    pushEmail(existing.assignedTo?.email);
    for (const l of existing.assigneeLinks) pushEmail(l.user.email);
    if (authorEmail) notifySet.delete(authorEmail);
    const to = [...notifySet];
  ...
  r.post("/projects/:projectId/rfis", needUser, async (c) => {
    ...
    const effectiveAssigneeIds = [
      ...new Set(
        body.data.assigneeUserIds?.length
    ...
    const draw = await resolveDrawingFields(
      ...
      body.data.pageNumber ?? null,
      body.data.pinNormX ?? null,
      body.data.pinNormY ?? null,
    );
  ...
  r.patch("/projects/:projectId/rfis/:rfiId", needUser, async (c) => {
    ...
    if (existing.status === RfiStatus.CLOSED) {
      const mayReopen =
        isRfiCreator(existing, userId) || admin || canRespond(existing, userId, false);
      if (data.status === RfiStatus.IN_REVIEW && mayReopen) {
        ...
        const recipientEmails = [
          ...new Set(
            [row.creator?.email, ...row.assigneeLinks.map((l) => l.user.email)].filter(
    ...
    if (data.answerMessageId === null) {
      const otherProvided = Object.entries(data).filter(
        ([k, v]) => v !== undefined && k !== "answerMessageId",
      );
      if (otherProvided.length > 0) {
        return c.json(
          { error: 'To remove the recorded answer, send only { "answerMessageId": null }' },
          400,
        );
      }
      ...
      if (!isRfiCreator(existing, userId) && !admin && !canRespond(existing, userId, false)) {
        return c.json(
          {
            error:
              "Only the creator, a responder, or a workspace admin can remove the recorded answer",
          },
          403,
        );
      }
    ...
    let answerMessageForPatch: { id: string; body: string } | null = null;
    ...
    if (statusRequested !== undefined && statusRequested !== existing.status) {
      if (statusRequested === RfiStatus.IN_REVIEW) {
        ...
        const nextAssigneeIds: string[] | null =
          data.assigneeUserIds !== undefined
            ...
            : data.assignedToUserId !== undefined
              ? data.assignedToUserId
                ...
                : []
              : null;
        const willHaveResponder =
          nextAssigneeIds !== null ? nextAssigneeIds.length > 0 : rfiHasResponder(existing);
        ...
        if (!canRespond(existing, userId, admin)) {
          return c.json(
            { error: "Only a responder (or a workspace admin) can mark the answer" },
            403,
          );
        }
        ...
        if (!mid) {
          return c.json(
            { error: "Select a discussion message as the official answer (answerMessageId)" },
            400,
          );
        }
    ...
    let assigneeIdsToSync: string[] | undefined;
    ...
    if (
      ...
      const draw = await resolveDrawingFields(
        ...
        data.pageNumber !== undefined ? data.pageNumber : existing.pageNumber,
        data.pinNormX !== undefined ? data.pinNormX : existing.pinNormX,
        data.pinNormY !== undefined ? data.pinNormY : existing.pinNormY,
      );
      if ("error" in draw) return c.json({ error: draw.error }, 400);
      drawPatch = draw;
    }
    ...
    if (statusRequested === RfiStatus.IN_REVIEW && existing.status === RfiStatus.OPEN) {
      ...
      const to = [
        ...new Set(
          row.assigneeLinks.map((l) => l.user.email).filter((e): e is string => !!e?.trim()),
        ),
      ];
      if (to.length > 0) {
  ...
  r.post("/projects/:projectId/rfis/:rfiId/attachments/presign", needUser, async (c) => {
    ...
    if (!rfi) return c.json({ error: "Not found" }, 404);
    if (rfi.status === RfiStatus.CLOSED) return c.json({ error: "RFI is closed" }, 400);
  ...
  r.get(
    "/projects/:projectId/rfis/:rfiId/attachments/:attachmentId/presign-read",
    needUser,
    async (c) => {
      const projectId = c.req.param("projectId")!;
      const rfiId = c.req.param("rfiId")!;
  ...
  r.post("/internal/rfi-overdue-reminders", async (c) => {
    ...
    for (const row of rows) {
      ...
      if (last && last.toISOString().slice(0, 10) === dayKey) continue;
