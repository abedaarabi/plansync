...
function parseDrawingTakeoffLinesFromViewerBlob(
  blob: unknown,
  fileId: string,
  fileVersionId: string,
  fileVersion: number,
  ...
  for (const it of items) {
    ...
    if (typeof r.id === "string") itemById.set(r.id, r);
  }
  const out: DrawingDerivedLine[] = [];
  for (const z of zones) {
    if (!z || typeof z !== "object") continue;
    const zr = z as Record<string, unknown>;
    if (typeof zr.id !== "string" || typeof zr.itemId !== "string") continue;
    const item = itemById.get(zr.itemId);
    const qty =
      typeof zr.computedQuantity === "number" ? zr.computedQuantity : Number(zr.computedQuantity);
    ...
    const notes =
      (typeof zr.notes === "string" && zr.notes.trim()) ||
...
async function buildProjectDrawingSyncSource(projectId: string): Promise<{
  lines: DrawingDerivedLine[];
  ...
  const latestByFile = new Map<string, (typeof versions)[number]>();
  for (const fv of versions) {
    if (!latestByFile.has(fv.fileId)) latestByFile.set(fv.fileId, fv);
  }
  const lines: DrawingDerivedLine[] = [];
  const sourceFileVersionIds: string[] = [];
  for (const fv of latestByFile.values()) {
    sourceFileVersionIds.push(fv.id);
    lines.push(
      ...parseDrawingTakeoffLinesFromViewerBlob(fv.annotationBlob, fv.fileId, fv.id, fv.version),
    );
  }
...
export function registerTakeoffRoutes(r: Hono, needUser: MiddlewareHandler) {
  ...
  r.get("/projects/:projectId/takeoff-lines", needUser, async (c) => {
    ...
    for (const fv of versions) {
      if (!latestByFile.has(fv.fileId)) latestByFile.set(fv.fileId, fv.version);
    }
    return c.json(
      rows.map((row) => {
        ...
        const mismatch =
          row.sourceFileVersionAtCreate != null &&
          latest != null &&
  ...
  r.post("/projects/:projectId/takeoff/sync/preview", needUser, async (c) => {
    ...
    for (const row of currentZoneRows) {
      if (!row.sourceZoneId || byZone.has(row.sourceZoneId)) continue;
      removed += 1;
    }
    return c.json({
      mode: "merge",
  ...
  r.post("/projects/:projectId/takeoff/sync/apply", needUser, async (c) => {
    ...
    if (body.data.mode === "replace") {
      const keepIds =
        body.data.protectManualEdits === false
          ? []
  ...
  r.post("/projects/:projectId/takeoff/views", needUser, async (c) => {
    ...
    const row = await prisma.takeoffViewPreset.create({
      data: {
        ...
        configJson: body.data.configJson as Prisma.InputJsonValue,
      },
    });
    return c.json(row);
  });
  ...
  r.delete("/projects/:projectId/takeoff/views/:viewId", needUser, async (c) => {
    ...
    if (d.count === 0) return c.json({ error: "View not found" }, 404);
  ...
  r.post("/projects/:projectId/takeoff/snapshots/:snapshotId/restore", needUser, async (c) => {
    ...
    const payload = snap.payloadJson as { lines?: Array<Record<string, unknown>> };
    ...
    for (const row of lines) {
      ...
      await prisma.takeoffLine.create({
        data: {
          ...
          sourceFileVersionAtCreate:
            row.sourceFileVersionAtCreate == null ? null : Number(row.sourceFileVersionAtCreate),
