      ...
      qRaw && qRaw.length > 0
        ? {
            OR: [
              ...
              { hall: { contains: qRaw, mode: "insensitive" as const } },
              { rowLabel: { contains: qRaw, mode: "insensitive" as const } },
              { rack: { contains: qRaw, mode: "insensitive" as const } },
              { positionU: { contains: qRaw, mode: "insensitive" as const } },
  ...
  r.post("/projects/:projectId/om/inspection-runs/:runId/complete", needUser, async (c) => {
    ...
    try {
      ...
      if (msg.startsWith("WORK_ORDER_CREATE_FAILED:")) {
        return c.json(
          { error: msg.slice("WORK_ORDER_CREATE_FAILED:".length) || "Failed to create work order" },
          400,
        );
      }
