...
import { getSheetAiPageFromDb, saveSheetAiChatToDb, saveSheetAiSummaryToDb, } from "../../lib/sheetAiCacheDb.js";
import { assertImageSizeOk, sheetChatBodySchema, sheetSummaryBodySchema, } from "../../lib/sheetAiSchemas.js";
...
async function authorizeSheetAi(fileVersionId, userId) {
...
export function registerSheetAiRoutes(r, needUser, env) {
    r.get("/file-versions/:fileVersionId/ai/sheet-cache", needUser, async (c) => {
        ...
        if ("error" in authz && authz.status === 404)
            return c.json({ error: authz.error }, 404);
        if ("error" in authz)
    ...
    r.post("/file-versions/:fileVersionId/ai/sheet-summary", needUser, aiBodyLimit, async (c) => {
        if (!geminiConfigured(env))
            return c.json({ error: "Sheet AI is not configured" }, 503);
        const fileVersionId = c.req.param("fileVersionId");
        const authz = await authorizeSheetAi(fileVersionId, c.get("user").id);
        if ("error" in authz && authz.status === 404)
            return c.json({ error: authz.error }, 404);
        ...
        if (!img.ok)
            return c.json({ error: img.error }, 400);
        try {
            const { summaryMarkdown, readingsTable, tableOfContents } = await geminiSheetSummary(env, parsed.data);
    ...
    r.post("/file-versions/:fileVersionId/ai/chat", needUser, aiBodyLimit, async (c) => {
        ...
        try {
            ...
            const thread = [...parsed.data.messages, { role: "model", content: reply }];
