...
export function v1Routes(auth, env, deps) {
    ...
    r.get("/me", needUser, async (c) => {
        ...
        const projectCounts = workspaceIds.length > 0
    ...
    r.patch("/workspaces/:workspaceId", needUser, async (c) => {
        ...
        if (p.website !== undefined) {
            ...
            if (raw === null || raw === "") {
                ...
                if (!n.ok)
                    return c.json({ error: n.message }, 400);
                nextWebsite = n.url;
            }
        }
        let nextLogo;
        ...
        if (env.STRIPE_SECRET_KEY && admin.workspace.stripeSubscriptionId) {
            ...
            try {
                ...
                if (code !== "resource_missing" && !/canceled|cancelled/i.test(msg)) {
                    ...
                    return c.json({
                        error: "Could not cancel the Stripe subscription. Cancel billing in Stripe first, then try again.",
                    }, 502);
                }
            }
        }
    ...
    r.post("/workspaces/:workspaceId/email-invites", needUser, async (c) => {
        ...
        if (!env.RESEND_API_KEY || !from) {
            return c.json({ error: "Email is not configured (set RESEND_API_KEY and RESEND_FROM)." }, 503);
        }
    ...
    r.post("/workspaces/:workspaceId/projects", needUser, async (c) => {
        ...
        let localBudget = null;
        if (body.data.localBudget != null &&
            body.data.localBudget !== "" &&
            !(typeof body.data.localBudget === "number" && Number.isNaN(body.data.localBudget))) {
            const raw = typeof body.data.localBudget === "number"
                ? body.data.localBudget
    ...
    r.patch("/projects/:projectId/files/:fileId", needUser, async (c) => {
        ...
        if (conflict) {
            return c.json({ error: "A file with this name already exists in the destination folder." }, 409);
        }
    ...
    r.post("/projects/:projectId/uploads/preview", needUser, async (c) => {
        ...
        const latestIds = files.map((f) => f.versions[0]?.id).filter((id) => Boolean(id));
        const grouped = latestIds.length > 0
            ...
            : [];
        const issueCountByVersionId = new Map(grouped.map((row) => [row.fileVersionId, row._count._all]));
        ...
        const rows = body.data.candidates.map((candidate) => {
            ...
            return {
                ...
                kind: isNewVersion ? "new_version" : "new_sheet",
                ...
                issueCountOnLatestVersion: latest && isNewVersion ? (issueCountByVersionId.get(latest.id) ?? 0) : 0,
            };
        });
        return c.json({ rows });
    });
    ...
    r.post("/files/upload", needUser, bodyLimit({
        ...
        const folderId = typeof parsed["folderId"] === "string" && parsed["folderId"].length > 0
            ? parsed["folderId"]
            : undefined;
        const fileName = typeof parsed["fileName"] === "string" && parsed["fileName"].length > 0
            ? parsed["fileName"]
            : uploaded.name;
        const fields = z
            .object({
    ...
    r.post("/files/complete-upload", needUser, async (c) => {
        ...
        if (!s3KeyMatchesFileUpload(body.data.s3Key, body.data.workspaceId, body.data.projectId, file.id)) {
    ...
    r.get("/files/:fileId/content", needUser, async (c) => {
        ...
        if (!obj.ok) {
            if (obj.error === "S3 not configured")
                return c.json({ error: obj.error }, 503);
    ...
    r.get("/workspaces/:workspaceId/dashboard", needUser, async (c) => {
        ...
        const [openIssues, projects, recent, fileCount, memberCount, activityForTrend] = await Promise.all([
        ...
        return c.json({
            ...
            issuesByStatus: openIssues.map((row) => ({
                status: row.status,
                _count: typeof row._count === "number" ? row._count : row._count._all,
            })),
    ...
    r.get("/projects/:projectId/team", needUser, async (c) => {
        ...
        const rows = buildProjectTeamMembers(wsMembers.map((m) => ({
            userId: m.userId,
            name: m.user.name,
            email: m.user.email,
            image: m.user.image,
    ...
    r.delete("/projects/:projectId/members/:userId", needUser, async (c) => {
        ...
        if (!pm) {
            return c.json({
                error: "This user is not scoped to this project only — remove them from the workspace under Organization, or adjust project access.",
            }, 400);
        }
    ...
    r.patch("/projects/:projectId", needUser, async (c) => {
        ...
        if (b.operationsMode !== undefined) {
            ...
            if (b.operationsMode === true && !isWorkspaceOmBilling(project.workspace)) {
                return c.json({
                    error: "Operations & Maintenance mode requires PlanSync Enterprise. Upgrade under Dashboard → Billing.",
                }, 402);
            }
        ...
        if (b.localBudget !== undefined) {
            if (b.localBudget === null || b.localBudget === "") {
                ...
                const raw = typeof b.localBudget === "number"
                    ? b.localBudget
        ...
        if (b.progressPercent !== undefined)
            data.progressPercent = b.progressPercent;
        ...
        const nextEndDate = b.endDate === undefined
            ? project.endDate
            : b.endDate === null
                ? null
