...
import { z } from "zod";
import { ActivityType, AssetMeterType, InspectionRunStatus, IssueKind, IssuePriority, IssueStatus, WorkOrderType, } from "@prisma/client";
...
import { parsePartsUsedJson, parseWorkOrderProcedure, parseWorkOrderProcedureResults, partsUsedToJsonValue, procedureResultsToJsonValue, validateProcedureCompletion, } from "../../lib/workOrderChecklist.js";
...
function requireOmBilling(workspace) {
    if (!isWorkspaceOmBilling(workspace)) {
        return { error: "O&M plan required", status: 402 };
    }
    return null;
}
function zodErrorMessage(flatten) {
    const fieldMsg = Object.values(flatten.fieldErrors)
        .flat()
        .find((m) => typeof m === "string" && m.length > 0);
    if (fieldMsg)
        return fieldMsg;
    const formMsg = flatten.formErrors.find((m) => typeof m === "string" && m.length > 0);
    return formMsg ?? "Invalid request";
}
async function getDefaultFileVersionForProject(projectId) {
...
export function registerWorkOrderRoutes(r, needUser, env) {
    // --- Vendors ---
    r.get("/projects/:projectId/om/vendors", needUser, async (c) => {
        ...
        if (auth.ctx.workspaceMember.isExternal)
    ...
    r.post("/projects/:projectId/om/vendors", needUser, async (c) => {
        ...
        if (!body.success)
            return c.json({ error: zodErrorMessage(body.error.flatten()) }, 400);
    ...
    r.post("/projects/:projectId/om/work-orders/from-occupant/:occupantIssueId", needUser, async (c) => {
        const projectId = c.req.param("projectId");
        ...
        if (!occ)
            return c.json({ error: "Tenant request not found" }, 404);
        if (!occ.assetId) {
            return c.json({ error: "Tenant request has no linked asset — assign equipment first" }, 400);
        }
        const body = z
            .object({
        ...
        if (body.data.assigneeId) {
            const a = await assertUserAssignableToProject(body.data.assigneeId, projectId, ctx.project.workspaceId);
            if ("error" in a)
    ...
    r.post("/projects/:projectId/om/work-orders/:issueId/complete", needUser, async (c) => {
        ...
        if (checklistErr)
            return c.json({ error: checklistErr }, 400);
        ...
        const now = new Date();
        const sourceRunId = issue.workOrderType === WorkOrderType.INSPECTION_FOLLOWUP
        ...
        const { updated, reInspectRunId } = await prisma.$transaction(async (tx) => {
            ...
            const wo = await tx.issue.update({
                ...
                data: {
                    ...
                    procedureResultJson: results.length > 0
                    ...
                    partsUsedJson: partsUsed.length > 0
    ...
    r.post("/projects/:projectId/om/work-orders/:issueId/ai-troubleshoot", needUser, async (c) => {
        ...
        try {
            const result = await troubleshootWorkOrderWithAi(env, {
                ...
                documentLabels: issue.asset.documents.map((d) => d.label.trim() ? d.label.trim() : d.fileName),
            });
    ...
    r.post("/projects/:projectId/om/assets/:assetId/meter-readings", needUser, async (c) => {
        ...
        const defaultFv = settings.modules.issues && auth.ctx.project.operationsMode
            ? await getDefaultFileVersionForProject(projectId)
            : null;
        for (const sched of schedules) {
            if (!sched.meterThreshold || readingValue < Number(sched.meterThreshold))
                continue;
            await logActivitySafe(auth.ctx.project.workspaceId, ActivityType.MAINTENANCE_SCHEDULE_UPDATED, {
                actorUserId: c.get("user").id,
    ...
    r.get("/projects/:projectId/om/reports/maintenance", needUser, async (c) => {
        ...
        const weekBuckets = new Map(weekKeys.map((weekStart) => [
            weekStart,
        ...
        const backlogPriorityCounts = new Map();
        ...
        const backlogByPriority = [IssuePriority.LOW, IssuePriority.MEDIUM, IssuePriority.HIGH].map((priority) => ({
            priority,
...
export function registerVendorWorkOrderPublicRoutes(r) {
