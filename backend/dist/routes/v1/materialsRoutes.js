...
export function registerMaterialsRoutes(r, needUser) {
    ...
    r.post("/workspaces/:workspaceId/materials", needUser, async (c) => {
        ...
        if (!typeKey || !nameKey)
            return c.json({ error: "Material type and name are required" }, 400);
        let unitPrice = null;
        if (body.data.unitPrice != null && body.data.unitPrice !== "") {
            const raw = typeof body.data.unitPrice === "number"
                ? body.data.unitPrice
    ...
    r.patch("/workspaces/:workspaceId/materials/:materialId", needUser, async (c) => {
        ...
        if (body.data.materialType !== undefined) {
            const typeKey = normalizeMaterialKey(body.data.materialType);
            if (!typeKey)
                return c.json({ error: "Material type is required" }, 400);
        ...
        const updated = await prisma.material.update({
            ...
            data: {
                ...
                unitPrice,
                currency: body.data.currency !== undefined
                    ? body.data.currency.trim().toUpperCase() || "USD"
                    : undefined,
                supplier: body.data.supplier !== undefined ? body.data.supplier?.trim() || null : undefined,
                manufacturer: body.data.manufacturer !== undefined ? body.data.manufacturer?.trim() || null : undefined,
                specification: body.data.specification !== undefined
