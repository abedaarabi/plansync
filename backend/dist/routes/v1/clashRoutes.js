...
async function authorizeClashProject(projectId, userId) {
    const auth = await loadProjectWithAuth(projectId, userId);
    if ("error" in auth)
        return auth;
    const pro = requireBimPro(auth.ctx.project.workspace);
    if (pro)
        return { error: pro.error, status: pro.status };
    return auth;
}
async function loadClashForUser(clashId, userId) {
...
export function registerClashRoutes(r, needUser, _env) {
    void _env;
    ...
    r.get("/clash-tests/:testId/clashes", needUser, async (c) => {
        ...
        const where = { testId };
        if (status && Object.values(BimClashStatus).includes(status)) {
            where.status = status;
        }
        if (assignee === "me")
            where.assigneeId = c.get("user").id;
    ...
    r.patch("/clashes/:clashId", needUser, async (c) => {
        ...
        if (body.data.assigneeId) {
            const ok = await assertUserAssignableToProject(body.data.assigneeId, loaded.clash.projectId, loaded.ctx.project.workspaceId);
            if ("error" in ok)
                return c.json({ error: ok.error }, ok.status);
        }
        const now = new Date();
        const statusChanging = body.data.status !== undefined && body.data.status !== loaded.clash.status;
        const updated = await prisma.bimClash.update({
            ...
            data: {
                ...
                ...(statusChanging
                    ? {
                        statusChangedAt: now,
                        statusDistanceMm: body.data.status === "RESOLVED" || body.data.status === "IGNORED"
    ...
    r.post("/clash-tests/:testId/clashes/bulk", needUser, async (c) => {
        ...
        if (!body.success)
            return c.json({ error: body.error.flatten() }, 400);
        if (body.data.assigneeId) {
            const ok = await assertUserAssignableToProject(body.data.assigneeId, test.projectId, auth.ctx.project.workspaceId);
            if ("error" in ok)
                return c.json({ error: ok.error }, ok.status);
        }
        const now = new Date();
        ...
        await Promise.all(clashes.map((clash) => {
    ...
    r.get("/buildings/:buildingId/clash-summary", needUser, async (c) => {
        ...
        const newestModelAt = ifcFiles.reduce((acc, f) => {
        ...
        for (const g of typeGroups) {
            if (g.clashType in byType) {
                byType[g.clashType] = g._count._all;
            }
        }
        ...
        const openByTest = new Map();
        const totalByTest = new Map();
        ...
        for (const t of tests) {
            if (t.lastRunAt && (!lastRunAt || t.lastRunAt > lastRunAt))
                lastRunAt = t.lastRunAt;
        }
        if (!lastRunAt && lastSeen._max.lastSeenAt)
            lastRunAt = lastSeen._max.lastSeenAt;
        const stale = Boolean(newestModelAt && lastRunAt && newestModelAt.getTime() > lastRunAt.getTime());
