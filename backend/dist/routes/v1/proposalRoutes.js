...
export function registerProposalRoutes(r, needUser, env) {
    ...
    r.get("/projects/:projectId/proposals", needUser, async (c) => {
        ...
        for (const row of list) {
            ...
            if (row.status === ProposalStatus.ACCEPTED)
                accepted += 1;
            if (row.status === ProposalStatus.SENT || row.status === ProposalStatus.VIEWED)
                sent += 1;
            if (row.status === ProposalStatus.DRAFT)
                draft += 1;
            if (row.status === ProposalStatus.DECLINED)
                declined += 1;
        }
        return c.json({
    ...
    r.patch("/projects/:projectId/proposals/:proposalId", needUser, async (c) => {
        ...
        if (body.data.clientName !== undefined)
            data.clientName = body.data.clientName;
        if (body.data.clientEmail !== undefined)
            data.clientEmail = body.data.clientEmail;
        if (body.data.clientCompany !== undefined)
    ...
    r.post("/projects/:projectId/proposals/:proposalId/items/sync-from-takeoff", needUser, async (c) => {
        const projectId = c.req.param("projectId");
        const proposalId = c.req.param("proposalId");
        const access = await loadProjectForMember(projectId, c.get("user").id);
        if ("error" in access)
        ...
        async function assertFvsInProject(fvIds) {
        ...
        if (mode === "replace") {
            ...
            const byTakeoff = new Map(existing.filter((x) => x.sourceTakeoffLineId).map((x) => [x.sourceTakeoffLineId, x]));
            for (const tl of lines) {
    ...
    r.post("/projects/:projectId/proposals/:proposalId/send", needUser, async (c) => {
        ...
        if (p.items.length === 0)
            return c.json({ error: "Add line items before sending" }, 400);
        ...
        const updated = await prisma.$transaction(async (tx) => {
            ...
            await tx.proposalRevision.create({
                data: { proposalId, snapshot: snapshot },
            });
            return u;
        });
    ...
    r.post("/projects/:projectId/proposals/:proposalId/resend", needUser, async (c) => {
        ...
        if (!p)
            return c.json({ error: "Not found" }, 404);
        if (!p.publicToken)
            return c.json({ error: "Proposal was never sent" }, 400);
    ...
    r.post("/workspaces/:workspaceId/proposal-templates", needUser, async (c) => {
        ...
        const t = await prisma.proposalTemplate.create({
            data: {
                ...
                name: body.data.name,
                body: body.data.body,
                defaultsJson: body.data.defaultsJson !== undefined && body.data.defaultsJson !== null
                    ? body.data.defaultsJson
                    : undefined,
            },
        });
        return c.json({
        ...
        const updated = await prisma.proposalTemplate.update({
            ...
            data: {
                ...
                ...(body.data.defaultsJson !== undefined
                    ? {
                        defaultsJson: body.data.defaultsJson === null
                            ? Prisma.JsonNull
                            : body.data.defaultsJson,
                    }
                    : {}),
...
Rules: Do not invent quantities or prices. No legal guarantees. Keep under 400 words.`;
    ...
    r.post("/public/proposals/:token/accept", async (c) => {
        ...
        if (!clientEmail) {
            console.warn("[proposal] acceptance: no client email — skipping client confirmation email", p.id);
        }
        if (clientEmail) {
            try {
    ...
    r.post("/public/proposals/:token/decline", async (c) => {
        ...
        if (p.validUntil < new Date())
            return c.json({ error: "Expired" }, 410);
        if (p.status === ProposalStatus.ACCEPTED)
            return c.json({ error: "Already finalized" }, 400);
        if (p.status !== ProposalStatus.SENT && p.status !== ProposalStatus.VIEWED) {
    ...
    r.get("/projects/:projectId/proposals/analytics/summary", needUser, async (c) => {
        ...
        let sent = 0;
        for (const x of all) {
            if (x.status === ProposalStatus.ACCEPTED)
                accepted += 1;
            if (x.status === ProposalStatus.DECLINED)
                declined += 1;
            if (x.status === ProposalStatus.SENT || x.status === ProposalStatus.VIEWED)
                sent += 1;
        }
    ...
    r.post("/projects/:projectId/proposals/:proposalId/document-versions/:versionId/restore", needUser, async (c) => {
        const projectId = c.req.param("projectId");
        const proposalId = c.req.param("proposalId");
    ...
    r.delete("/projects/:projectId/proposals/:proposalId/comments/:commentId", needUser, async (c) => {
        const projectId = c.req.param("projectId");
        const proposalId = c.req.param("proposalId");
        const commentId = c.req.param("commentId");
        const access = await loadProjectForMember(projectId, c.get("user").id);
