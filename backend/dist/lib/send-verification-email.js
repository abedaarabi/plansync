 ...
 */
export function queueVerificationEmail(env, params) {
    void (async () => {
        ...
        const greet = displayName?.trim() != null && displayName.trim() !== ""
            ? `Hi ${displayName.trim()},`
            : "Hi,";
        const html = buildTransactionalEmailHtml(env, {
            ...
            footerNote: "If you didn't sign up for PlanSync, someone may have mistyped your address — you can ignore this email.",
        });
        ...
        console.error("[verification-email] RESEND_API_KEY and RESEND_FROM must be set in production to send verification emails.");
    })();
}
