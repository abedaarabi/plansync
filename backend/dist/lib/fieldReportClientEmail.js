...
function parseDetails(details) {
    ...
    const crews = Array.isArray(crewsRaw)
        ? crewsRaw
    ...
    const delays = Array.isArray(delaysRaw)
        ? delaysRaw
            ...
            .map((d) => ({ text: String(d.text ?? "").trim() }))
            .filter((d) => d.text.length > 0)
        : [];
    const matRaw = o.materials;
    const materials = Array.isArray(matRaw)
        ? matRaw
    ...
    const photos = Array.isArray(phRaw)
        ? phRaw
...
function weatherSummary(report, d) {
...
function buildDailyBlocksHtml(report, include) {
    const d = parseDetails(report.details);
    const parts = [];
    if (include.weather) {
        const w = weatherSummary(report, d);
        parts.push(sectionHtml("Weather", `<p style="margin:0;font-size:14px;line-height:1.6;color:#334155">${escapeHtml(w)}</p>`));
    }
    if (include.workers) {
        const lines = [];
        ...
        parts.push(sectionHtml("Workers", `<ul style="margin:0;padding-left:18px;font-size:14px;line-height:1.55;color:#334155">${lines
            .map((l) => `<li style="margin:4px 0">${escapeHtml(l)}</li>`)
            .join("")}</ul>`));
    }
    if (include.completed) {
        const items = [];
        for (const x of d.workCompleted)
            items.push(x.text);
        for (const x of d.workInProgress)
            items.push(`(In progress) ${x.text}`);
    ...
    if (include.photos) {
        ...
        const caps = d.photos.map((p) => p.caption).filter((c) => !!c && c.length > 0);
    ...
    if (notes) {
        parts.push(sectionHtml("Notes", `<p style="margin:0;font-size:14px;line-height:1.65;color:#334155">${escapeHtml(notes)}</p>`));
    }
    return parts.join("");
}
function buildWeeklyBlocksHtml(reports, include) {
    const sorted = [...reports].sort((a, b) => a.reportDate.toISOString().localeCompare(b.reportDate.toISOString()));
    return sorted
        .map((r) => {
...
export function buildFieldReportEmailHtml(env, input) {
    const msg = input.message?.trim();
    const bodyLines = [
        `${input.senderName} shared a field report for ${input.projectName}.`,
        msg ? `Message: ${msg}` : "",
    ].filter((l) => l.length > 0);
    const extra = input.mode === "daily" && input.reports[0]
...
export function buildFieldReportEmailText(input) {
    ...
    for (const report of [...input.reports].sort((a, b) => a.reportDate.toISOString().localeCompare(b.reportDate.toISOString()))) {
        if (input.mode === "weekly") {
            lines.push(`— ${formatYmd(report.reportDate.toISOString())}${report.authorLabel?.trim() ? ` · ${report.authorLabel.trim()}` : ""}`, "");
        }
        const d = parseDetails(report.details);
        ...
        if (input.include.delays) {
            pushSection("Delays and issues", d.delays.length ? d.delays.map((x) => x.text) : ["None recorded."]);
        }
        if (input.include.materials) {
            pushSection("Materials", d.materials.length ? d.materials.map((x) => x.text) : ["None recorded."]);
        }
        if (input.include.photos) {
            const n = report.photoCount ?? d.photos.length;
            const caps = d.photos.map((p) => p.caption).filter(Boolean);
            pushSection("Photos", [
