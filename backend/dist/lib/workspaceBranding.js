...
export function faviconUrlFromHostname(hostname) {
...
export function normalizeWorkspaceWebsite(input) {
    const t = input.trim();
    if (!t)
        return { ok: false, message: "Website cannot be empty" };
    ...
    if (!hostname)
        return { ok: false, message: "Invalid website" };
