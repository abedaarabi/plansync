...
export function buildPunchListPath(projectId, punchId) {
    const base = `/projects/${projectId}/punch`;
    if (!punchId)
        return base;
    return `${base}?punch=${encodeURIComponent(punchId)}`;
}
export function buildPunchListUrl(env, projectId, punchId) {
    const base = env.PUBLIC_APP_URL.replace(/\/$/, "");
    return `${base}${buildPunchListPath(projectId, punchId)}`;
}
export function buildPunchAssignedEmailHtml(env, input) {
...
export function buildPunchAssignedEmailText(input) {
    const bulk = typeof input.bulkCount === "number" && input.bulkCount > 1;
    if (bulk) {
...
`;
    ...
    const line = input.punchTitle && input.punchNumber != null
        ? `#${input.punchNumber} — ${input.punchTitle}`
