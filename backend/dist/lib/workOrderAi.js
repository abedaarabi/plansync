...
export async function troubleshootWorkOrderWithAi(env, input) {
...
Keep steps practical for a frontline technician. Max 6 steps and 4 safety notes.`;
    ...
    const suggestedSteps = Array.isArray(raw.suggestedSteps)
        ? raw.suggestedSteps.filter((s) => typeof s === "string" && s.trim().length > 0)
        : [];
    const safetyNotes = Array.isArray(raw.safetyNotes)
        ? raw.safetyNotes.filter((s) => typeof s === "string" && s.trim().length > 0)
        : [];
    if (!summary)
