...
export function buildGoogleAuthorizeUrl(env, state) {
    ...
    u.searchParams.set("scope", ["https://www.googleapis.com/auth/drive.readonly", "openid", "email", "profile"].join(" "));
...
export async function exchangeGoogleCode(env, code) {
    ...
    if (!res.ok) {
        return {
            error: typeof j.error_description === "string" ? j.error_description : "Token exchange failed",
        };
    }
...
async function listGoogleDriveFilesInParent(accessToken, parentInQuery, opts) {
...
async function listGoogleDriveByParentId(accessToken, parentId) {
...
async function listSharedDriveEntries(accessToken) {
...
export async function listGoogleDriveChildren(accessToken, parentId) {
...
export async function getGoogleDriveWebViewUrl(accessToken, fileId) {
    ...
    const res = await fetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?fields=webViewLink&supportsAllDrives=true`, { headers: { Authorization: `Bearer ${accessToken}` } });
    const j = (await res.json());
    if (!res.ok) {
        const hint = j.error?.message ? `: ${j.error.message}` : "";
...
export async function downloadGoogleDriveFile(accessToken, fileId) {
    const metaRes = await fetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?fields=mimeType,name`, { headers: { Authorization: `Bearer ${accessToken}` } });
    ...
    const res = await fetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(fileId)}?alt=media&supportsAllDrives=true`, { headers: { Authorization: `Bearer ${accessToken}` } });
    if (!res.ok)
        return { error: "Could not download file from Google Drive" };
    const ab = await res.arrayBuffer();
    return Buffer.from(ab);
}
// --- Microsoft OneDrive (Graph) ---
export function buildMicrosoftAuthorizeUrl(env, state) {
...
export async function exchangeMicrosoftCode(env, code) {
    const id = env.MICROSOFT_CLIENT_ID?.trim();
...
export async function refreshMicrosoftAccessToken(env, refreshToken) {
    const id = env.MICROSOFT_CLIENT_ID?.trim();
    const secret = env.MICROSOFT_CLIENT_SECRET?.trim();
    if (!id || !secret)
        return { error: "Microsoft OAuth is not configured" };
...
export async function listMicrosoftDriveChildren(accessToken, parentId) {
    const path = parentId == null || parentId === "root"
    ...
    const j = (await res.json());
    if (!res.ok)
        return { error: "Could not list OneDrive folder" };
...
export async function downloadMicrosoftDriveFile(accessToken, itemId) {
    const res = await fetch(`https://graph.microsoft.com/v1.0/me/drive/items/${encodeURIComponent(itemId)}/content`, { headers: { Authorization: `Bearer ${accessToken}` } });
    if (!res.ok)
        return { error: "Could not download file from OneDrive" };
    const ab = await res.arrayBuffer();
    return Buffer.from(ab);
}
/** Open in browser: OneDrive / SharePoint web URL for an item. */
export async function getMicrosoftDriveWebUrl(accessToken, itemId) {
    const res = await fetch(`https://graph.microsoft.com/v1.0/me/drive/items/${encodeURIComponent(itemId)}?$select=webUrl`, { headers: { Authorization: `Bearer ${accessToken}` } });
    const j = (await res.json());
    if (!res.ok) {
        const hint = j.error?.message ? `: ${j.error.message}` : "";
...
// --- Dropbox ---
export function buildDropboxAuthorizeUrl(env, state) {
...
export async function exchangeDropboxCode(env, code) {
    const id = env.DROPBOX_APP_KEY?.trim();
    const secret = env.DROPBOX_APP_SECRET?.trim();
    if (!id || !secret)
        return { error: "Dropbox OAuth is not configured" };
    const body = new URLSearchParams({
        code,
        grant_type: "authorization_code",
        client_id: id,
...
export async function refreshDropboxAccessToken(env, refreshToken) {
    const id = env.DROPBOX_APP_KEY?.trim();
    const secret = env.DROPBOX_APP_SECRET?.trim();
    if (!id || !secret)
        return { error: "Dropbox OAuth is not configured" };
...
export async function listDropboxFolder(accessToken, path) {
...
export async function downloadDropboxFile(accessToken, pathLower) {
    ...
    if (!res.ok)
        return { error: "Could not download file from Dropbox" };
    const ab = await res.arrayBuffer();
    return Buffer.from(ab);
}
/**
 ...
 */
export async function getDropboxOpenUrl(accessToken, pathLower) {
...
export async function fetchGoogleAccountLabel(accessToken) {
...
export async function fetchMicrosoftAccountLabel(accessToken) {
...
export async function fetchDropboxAccountLabel(accessToken) {
