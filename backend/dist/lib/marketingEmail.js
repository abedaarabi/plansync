import { EMAIL_RESPONSIVE_CSS, escapeHtml, escapeHtmlWithPlanSyncWordmark, planSyncBrandHeaderHtml, planSyncEmailIconPublicUrl, planSyncWordmarkHtml, } from "./transactionalEmailLayout.js";
...
function marketingPublicAssetUrl(appBase, assetPath) {
    return `${appBase.replace(/\/$/, "")}${assetPath.startsWith("/") ? assetPath : `/${assetPath}`}`;
}
function isLocalDevAppUrl(url) {
    try {
...
export function resolveMarketingAppUrl(publicAppUrl) {
...
function marketingAppDisplayHost(appBase) {
    try {
...
export function marketingEmailSubject() {
    return "From PDF drawing to sent proposal — 6 months of PlanSync Pro, free";
}
function greeting(recipient) {
    if (recipient.name?.trim())
        return `Hi ${recipient.name.trim()},`;
    if (recipient.company?.trim())
        return `Hi ${recipient.company.trim()} team,`;
    return "Hi there,";
}
function founderOpening(recipient) {
    return `${greeting(recipient)} I'm ${MARKETING_FOUNDER.name}, founder of PlanSync.`;
}
export function buildMarketingEmailText(recipient) {
...
function smallPreviewImagesHtml(appBase, signInUrl) {
...
function marketingDemoEmbedUrl(startSeconds = 92, origin) {
...
function videoPlayButtonHtml(size = 80) {
...
function demoVideoHtml(embedInline = false, embedOrigin) {
...
function workflowStepsHtml() {
    const stepCell = (s, showArrow) => `<td class="mkt-stack-col" style="padding:6px;vertical-align:top;width:33%">
    ...
    const cells = MARKETING_WORKFLOW.map((s, i) => stepCell(s, i < MARKETING_WORKFLOW.length - 1)).join("");
    return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr>${cells}</tr></table>`;
}
function founderNoteHtml(greet) {
...
function featureCardsHtml() {
    const featureCell = (f) => `<td class="mkt-stack-col" style="padding:6px;width:50%;vertical-align:top">
    ...
    for (let i = 0; i < MARKETING_FEATURES.length; i += 2) {
        ...
        rows.push(`<tr>${featureCell(left)}${right ? featureCell(right) : `<td class="mkt-stack-col" style="padding:6px;width:50%"></td>`}</tr>`);
    }
    return rows.join("");
}
function offerBannerHtml() {
    const perkCells = MARKETING_OFFER_PERKS.map((label, i) => `<td class="mkt-perk" style="padding:${i < MARKETING_OFFER_PERKS.length - 1 ? "0 12px 0 0" : "0"};font-size:12px;line-height:1.5;color:#cbd5e1;font-family:${FF}">✓ ${escapeHtml(label)}</td>`).join("");
...
function headerHtml(iconUrl, appUrl) {
...
function socialLinksHtml() {
    const pill = (href, label, accent = false) => `<td class="mkt-social-pill" style="padding:4px">
...
export function buildMarketingEmailHtml(recipient, opts) {
    ...
    const founderSignOff = escapeHtmlWithPlanSyncWordmark(`${MARKETING_FOUNDER.name}, ${MARKETING_FOUNDER.title}`);
...
function pickFieldRaw(row, keys) {
    for (const [rawKey, value] of Object.entries(row)) {
        const key = normalizeHeader(rawKey);
        if (keys.includes(key))
            return value;
...
export function parseMarketingSentFlag(value) {
...
export function markMarketingRowsSent(rows, rowIndices) {
    const updated = rows.map((row) => ({ ...row }));
    for (const idx of rowIndices) {
        const row = updated[idx];
        if (row)
            updated[idx] = { ...row, sent: true };
    }
    return updated;
}
/** Parse rows from Excel/CSV sheet objects (header row keys). */
export function parseMarketingRecipients(rows) {
    const recipients = [];
    const skipped = [];
    const seen = new Set();
    let alreadySent = 0;
    rows.forEach((row, index) => {
        ...
        const email = pickField(row, ["email", "e-mail", "email address", "work email", "contact email"]) ??
            Object.values(row)
                .map((v) => String(v ?? "").trim())
