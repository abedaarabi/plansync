...
export function assertImageSizeOk(imageBase64) {
...
export function uniqueTakeoffCategoriesInOrder(cats) {
    const seen = new Set();
    const out = [];
...
function ensureMinSpanTakeoff1D(min, max) {
    let a = clamp01(min);
    let b = clamp01(max);
    if (b < a)
        [a, b] = [b, a];
    const span = b - a;
    if (span >= TAKEOFF_ASSIST_MIN_CLICK_SPAN)
        return { min: a, max: b };
    const mid = (a + b) / 2;
...
const takeoffCategoryPreprocess = z.preprocess((v) => {
    const t = String(v ?? "")
        .toLowerCase()
        .trim();
    if (t === "window" || t === "win")
...
export const takeoffAssistResultSchema = z.object({
    counts: z
        ...
        .optional(),
    items: z.preprocess((v) => (Array.isArray(v) ? v : []), z
        .array(z.object({
...
function asFiniteNumberLoose(v) {
...
export function normalizeTakeoffCategoryLoose(v) {
    const s = String(v ?? "")
        ...
        .trim()
        .replace(/\s+/g, "_");
    if (s === "window" || s === "windows" || s === "win" || s === "w")
        return "windows";
    if (s === "door" || s === "doors" || s === "d")
        return "doors";
    if (s === "wall" || s === "walls")
        return "walls";
    if (s === "room" || s === "rooms" || s === "space" || s === "spaces")
        return "rooms";
    const p = takeoffAssistCategorySchema.safeParse(s);
    return p.success ? p.data : null;
}
function extractItemsArrayLoose(obj) {
    const keys = [
        "items",
        "detections",
        "boxes",
 ...
 */
export function parseTakeoffAssistResultLoose(raw) {
    let obj = null;
    let rows = null;
    ...
    for (const row of rows) {
        ...
        if (!category)
            continue;
        const bbox = r.bbox && typeof r.bbox === "object" && !Array.isArray(r.bbox)
            ? r.bbox
            : null;
        ...
        if (minX != null && maxX == null && w != null)
            maxX = minX + Math.abs(w);
        if (minY != null && maxY == null && h != null)
            maxY = minY + Math.abs(h);
...
function normalizeTakeoffCoordsToUnit(minX, minY, maxX, maxY) {
...
export function sanitizeTakeoffAssistResult(parsed, expectedPageIndex0, allowedCategories) {
    const allow = new Set(allowedCategories);
    const itemsIn = [];
    for (const it of parsed.items.slice(0, TAKEOFF_ASSIST_MAX_ITEMS)) {
        if (!allow.has(it.category))
            continue;
...
export const sheetSummaryResultSchema = z.object({
    ...
    readingsTable: z.preprocess((v) => (Array.isArray(v) ? v : []), z.array(readingRowRawSchema).max(45)),
    tableOfContents: z.preprocess((v) => (Array.isArray(v) ? v : []), z.array(tocEntryRawSchema).max(40)),
});
/** Only expand boxes that are unrealistically tiny (click target), not full detail callouts. */
const TOC_MIN_CLICK_SPAN = 0.014;
function ensureMinSpan1D(min, max) {
    let a = clamp01(min);
    let b = clamp01(max);
    if (b < a)
        [a, b] = [b, a];
    const span = b - a;
    if (span >= TOC_MIN_CLICK_SPAN)
        return { min: a, max: b };
    const mid = (a + b) / 2;
...
export function sanitizeSheetSummaryResult(parsed, expectedPageIndex0) {
    const summaryMarkdown = parsed.summaryMarkdown.trim();
    const readingsTable = parsed.readingsTable
        .map((r) => {
