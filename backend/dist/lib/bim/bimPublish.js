...
function storeysFromByLevel(byLevel) {
...
function conversionStoreysReady(status) {
    return status === "summary_ready" || status === "ready";
}
async function readQuantityIndexFromS3(env, s3Key) {
    try {
        const obj = await getObjectStream(env, s3Key);
        if (!obj.ok)
            return null;
...
export async function getStoreysResponseForFileVersion(env, fileVersionId) {
...
async function getStoreysForFileVersion(env, fileVersionId) {
