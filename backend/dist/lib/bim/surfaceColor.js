...
function isDefaultGrayRgb(r, g, b) {
    ...
    for (const [dr, dg, db] of grays) {
        if (Math.abs(R - dr) < 10 && Math.abs(G - dg) < 10 && Math.abs(B - db) < 10)
            return true;
    }
    const avg = (R + G + B) / 3;
...
export function extractSurfaceColorFromMaterials(materials) {
...
export function hasAuthoredSurfaceStyle(materials, surfaceColor) {
