...
export function assertIfcBytesIntact(bytes, fileLabel = "IFC file") {
    ...
    if (!/END-ISO-10303-21\s*;?/i.test(tail)) {
        throw new Error(`${fileLabel} looks incomplete or corrupted (file ends abruptly). Re-upload the full IFC.`);
    }
}
