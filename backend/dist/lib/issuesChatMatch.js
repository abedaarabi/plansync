...
const CLOSED_LIKE = new Set([IssueStatus.RESOLVED, IssueStatus.CLOSED]);
...
function issueDueDayStartMs(due) {
    if (!due)
        return null;
    return Date.UTC(due.getUTCFullYear(), due.getUTCMonth(), due.getUTCDate());
}
export function isIssueRowOverdue(row, nowMs) {
    if (CLOSED_LIKE.has(row.status))
        return false;
...
function issueLooksClashRelated(row, clashIssueIds) {
    if (clashIssueIds.has(row.id))
        return true;
    if (row.bimAnchor != null)
        return true;
    const blob = `${row.title ?? ""} ${row.description ?? ""}`.toLowerCase();
    return /\bclash(ed|es|ing)?\b|\bbim\b/.test(blob);
}
export function catalogRank(row, nowMs) {
    let score = 0;
    if (row.status === IssueStatus.OPEN)
        score += 4;
    else if (row.status === IssueStatus.IN_PROGRESS)
        score += 3;
    if (row.priority === IssuePriority.HIGH)
        score += 2;
    if (isIssueRowOverdue(row, nowMs))
        score += 2;
    return score;
}
function parseStatus(q) {
    if (/\bin progress\b|\bin_progress\b|\bactive\b/.test(q))
        return IssueStatus.IN_PROGRESS;
    if (/\bresolved\b|\bdone\b/.test(q))
...
function issueHaystack(row, displayNums) {
    return [
        row.title,
        row.description,
        row.location,
...
function parseIntent(q) {
    ...
    const wantMine = /\b(my issues|assigned to me|assigned me|assigned to myself|only mine)\b/.test(q) ||
        /\bmine\b/.test(q);
    ...
    const wantAll = /\b(all|every|entire|full)\b/.test(q) ||
        /\b(give|show|list|get)\b.+\bissues?\b/.test(q) ||
        /^(issues?|all issues?)$/.test(q);
    const hasFilter = wantOverdue ||
        wantMine ||
        wantUnassigned ||
        wantHigh ||
        wantMedium ||
...
function passesStructuredFilters(row, intent, userId, nowMs, clashIssueIds) {
    const checks = [
...
export function matchIssuesForQuery(rows, displayNums, query, userId, nowMs, clashIssueIds) {
    const q = query.trim().toLowerCase();
    if (!q)
        return { matched: [], totalMatched: 0 };
    const intent = parseIntent(q);
    const filtered = rows.filter((row) => {
        if (!passesStructuredFilters(row, intent, userId, nowMs, clashIssueIds))
            return false;
        if (intent.displayNum != null && displayNums.get(row.id) === intent.displayNum)
            return true;
    ...
    const result = filtered.length === 0 && intent.wantAll
