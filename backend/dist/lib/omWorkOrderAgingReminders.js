...
import { createOmDigestResend, emptyDigestCounters, sendDigestResendEmail, tryNotify, } from "./omReminderDigestShared.js";
function workOrdersAppHref(projectId) {
...
export async function runOmWorkOrderAgingReminders(env) {
    ...
    for (const issue of issues) {
        ...
        if (!settings.modules.omMaintenance && !settings.modules.issues)
            continue;
    ...
    for (const [workspaceId, counts] of byWorkspace) {
        ...
        const notifTitle = `Work orders aging: ${row.over7} over 7d` +
            (row.over30 > 0 ? `, ${row.over30} over 30d` : "");
        const notifBody = `${row.workspaceName}: ${row.over7} open/in-progress work order(s) older than 7 days (UTC)` +
