...
export async function loadProjectWithAuth(projectId, userId) {
    ...
    const canAccessAnyProjectInWorkspace = !wm.isExternal && (wm.role === WorkspaceRole.SUPER_ADMIN || wm.role === WorkspaceRole.ADMIN);
    if (limited.length > 0 &&
 ...
 */
export function isProjectAccessError(access) {
