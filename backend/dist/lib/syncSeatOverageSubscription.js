...
import { includedSeatsForBillingPlan } from "../config/product.js";
import { prisma } from "./prisma.js";
import { countSeatPressure, seatOverageQuantity } from "./seatPressure.js";
import { isWorkspacePro } from "./subscription.js";
import { isExtraSeatSubscriptionItem, resolveExtraSeatMonthlyPriceId, } from "./stripeExtraSeatPrice.js";
...
async function deleteSubscriptionItems(stripe, items) {
    for (const item of items) {
        await stripe.subscriptionItems.del(item.id, PRORATE);
    }
}
async function setSeatOverageQuantity(stripe, subscriptionId, matching, seatPriceId, overage, clearedObsolete) {
    if (overage === 0) {
        if (matching) {
 ...
 */
export async function syncSeatOverageSubscription(stripe, env, workspace) {
    ...
    return setSeatOverageQuantity(stripe, sub.id, matching, seatPriceId, overage, obsolete.length > 0);
}
/** Best-effort sync after seat-pressure changes — never fails the caller. */
export async function syncWorkspaceSeatOverageSafe(env, workspaceId) {
    ...
    try {
        ...
        if (result.ok && result.action !== "noop") {
            console.info(`[stripe] seat overage sync workspace=${workspaceId} overage=${result.overage} action=${result.action}`);
        }
    }
    catch (e) {
