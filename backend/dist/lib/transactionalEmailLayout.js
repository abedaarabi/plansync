...
export function publicApiBaseFromEnv(env) {
    const app = env.PUBLIC_APP_URL.replace(/\/$/, "");
    return env.PUBLIC_API_URL?.replace(/\/$/, "") || app;
}
export function planSyncEmailIconUrl(env) {
...
export function buildTransactionalEmailHtml(env, content) {
    ...
    const linesHtml = content.bodyLines
        ...
        .map((l) => `<p style="margin:0 0 12px;font-size:15px;line-height:1.65;color:#334155;font-family:${FF}">${escapeHtml(l)}</p>`)
        .join("");
