 ...
 */
export function escapeHtmlWithPlanSyncWordmark(s, opts) {
...
function planSyncEmailIconUrl(env) {
...
export function planSyncBrandHeaderHtml(iconUrl, tagline = "Construction collaboration", rightHtml, appUrl) {
 ...
 */
export function buildTransactionalEmailHtml(env, content) {
    ...
    const linesHtml = content.bodyLines
        ...
        .map((l) => `<p style="margin:0 0 12px;font-size:15px;line-height:1.65;color:${C.body};font-family:${FF}">${escapeHtmlWithPlanSyncWordmark(l, { planColor: C.body })}</p>`)
        .join("");
